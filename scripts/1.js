...
function InventoryHistory_LoadMore()
{
	...
	$J.ajax({
		...
		else
		{
			...
			if ( data.error )
			{
				ShowAlertDialog( 'Ошибка', data.error, 'OK' );
			}
		}
	}).fail( function( jqXHR ) {
		g_historyCursor = prevCursor;
		...
		if ( jqXHR.status == 429 )
		{
			ShowAlertDialog( 'Ошибка', 'Вы делали слишком много запросов. Пожалуйста, подождите и повторите запрос позже.', 'OK' );
		}
		else
		{
			ShowAlertDialog( 'Ошибка', 'Во время загрузки истории инвентаря произошла ошибка.', 'OK' );
		}
	}).always( function() {
...
function InventoryHistory_InitMessages()
{
	$J('.inventory_history_message').each( function( index ) {
		...
		{
			var $elMore = $J( '<div class="showmore"><span>Ещё</span></div>' );
...
HistoryFiltersDialog = {
	...
	Show: function () {
		...
		var _this = this;
		this.m_modal = ShowDialog( 'Фильтры', $J('#inventory_history_filters_dialog' ).show() );
